(function() {
	...
	function loadItems(url, params, errorMsg) {
		fetch(`${url}?${params}`)
			...
			.then(items => {
				console.log("Items fetched from API:", items); // Debugging log
				if (!items || items.length === 0) {
					showErrorMessage(errorMsg);
				} else {
					listItems(items); // Call the fixed listItems
				}
			})
			.catch(error => {
				console.error("Error fetching items:", error);
				showErrorMessage('Unable to fetch items.');
			});
	}
	...
	function toggleFavorite(event) {
	  ...
	  const payload = JSON.stringify({ user_id: '1111', favorite: [item_id] });
