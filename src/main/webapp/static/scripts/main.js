(function () {
	...
	function loadNearbyItems() {
		loadItems('https://52.204.138.195:8443/EventRecommender/search', `user_id=${user_id}&lat=${lat}&lon=${lng}`, 'Nearby items not found.');
	}

	function loadFavoriteItems() {
		loadItems('https://52.204.138.195:8443/EventRecommender/history', `user_id=${user_id}`, 'No favorite items found.', true);
	}

	function loadRecommendedItems() {
		loadItems('https://52.204.138.195:8443/EventRecommender/recommendation', `user_id=${user_id}&lat=${lat}&lon=${lng}`, 'No recommended items found.');
	}
	...
	function listItems(items, isFavoriteView = false) {
		...
		itemList.innerHTML = '';
		items.forEach(item => {
			const isFavorite = isFavoriteView || item.favorite === true; // Ensure correct state in favorites view
	...
	function toggleFavorite(event) {
		...
		const url = 'https://52.204.138.195:8443/EventRecommender/history';
